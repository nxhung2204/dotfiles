...
5. **Step-by-step Guide**: Giữ ngắn gọn — chỉ gồm:
   - File paths cần tạo/sửa
   - Key decisions/constraints (e.g. "dùng `<slot />` trong Layout, không dùng per-route meta")
   - Tối đa 1-2 code snippet nhỏ cho phần non-obvious (không paste toàn bộ component)
   - Không viết full implementation code — đó là việc của implement agent
